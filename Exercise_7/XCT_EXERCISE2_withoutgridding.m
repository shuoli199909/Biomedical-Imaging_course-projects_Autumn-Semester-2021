...
function [] = XCT_EXERCISE2_shuo()
    ...
    for phi=projection_angles
        ...
        disp(length(projection_angles)-idx);
        projection = phantom.sino(:,length(projection_angles)+1-idx);
        projection = repmat(projection',matrix+1,1);
        
        for x = -128:128
            for y = -128:128
                r_ = round(x*cos(phi*pi/180) + y*sin(phi*pi/180));
                s_ = round(x*sin(phi*pi/180) - y*cos(phi*pi/180));
                if (s_<=128 && s_>=-128)&&(r_<=128 && r_>=-128)
                    s_ = s_ + 129;
                    r_ = r_ + 129;
                else
                    continue;
                end
                phantom.sbp(x+129,y+129) = phantom.sbp(x+129,y+129) + projection(r_,s_);
            end
        end
        
        DisplayData(phantom.sbp,[2,3,4]); 
        title('Backprojection-without gridding'); xlabel('x'); ylabel('y'); drawnow;
        idx = idx+1;                                        % counter
    end  
    ...
    for phi=projection_angles
        ...
        projection = U2R(projection_fourier.*filter');
        projection = repmat(projection',matrix+1,1);
        for r = -128:128            
            for s = -128:128
                x_ = round(r*cos(phi*pi/180) + s*sin(phi*pi/180));
                y_ = round(r*sin(phi*pi/180) - s*cos(phi*pi/180));
                if (x_<=128 && x_>=-128)&&(y_<=128 && y_>=-128)
                    x_ = x_ + 129;
                    y_ = y_ + 129;
                else
                    continue;
                end
                phantom.fbp(x_,y_) = phantom.fbp(x_,y_) + projection(r+129,s+129);
            end            
        end           
        
        DisplayData(phantom.fbp,[2,3,5]); 
        title('Filtered Backprojection-without gridding'); xlabel('x'); ylabel('y'); drawnow;        
        idx = idx+1;                                        % counter
    end  
    ...
    for phi=projection_angles
        ...
        vec = [];
        for u = -128:128
            p_ = round(u*sin(phi*pi/180));
            q_ = round(u*cos(phi*pi/180));
            if (p_<=128 && p_>=-128)&&(q_<=128 && q_>=-128)
                p_ = p_ + 129;
                q_ = q_ + 129;
                vec = [vec;[p_,q_]];
            else
            ...
            phantom.fft(p_,q_) = phantom.fft(p_,q_) + projection_fourier(u+129);
        end
    ...
    title('2D Fourier transform-without gridding'); xlabel('x'); ylabel('y'); drawnow; 
end
